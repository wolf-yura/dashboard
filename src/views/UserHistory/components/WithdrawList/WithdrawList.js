...
const WithdrawList = props => {
  ...
  const { className, userId, UserService, AuthService, PlanService,WithdrawService, MySwal, ...rest } = props;
  //handle table
              ...
              <TableBody>
                {plans.slice((page) * rowsPerPage, (page + 1) * rowsPerPage).map(item => (
                  ...
                  >
                    ...
                    <TableCell>{item.status == 'concluído' ? 'Aprovação' : item.status == 'cancelado' ? 'Cancelado ' : 'Solicitação' }</TableCell>
                    <TableCell>
